...
dim adoCmd, adoParam1, adoParam2, adoParam3, adoParam4, adoParam5, adoParam6, adoParam7, adoParam8, adoParam9
...
if Request.Querystring("UserCriteria") = "Initial" then
  ...
  adoCmd.CommandTimeout = 0
	...
	'sSQL = sSQL & " n.FirstName + ' ' + n.MiddleName + ' ' + n.LastName ParticName," 
	'sSQL = sSQL & " n.SSN 'dbSSN' from phonecalls p inner join"
rem	sSQL = sSQL & " userinformation u on p.whotookcall= u.acsuserid left join names n on"
  rem live code is different on the next line
	'sSQL = sSQL & " userinformation u on p.whotookcall=convert(varchar(6),u.ACSUserTrackNumber) left join names n on"
  ...
  adoCmd.CommandText = "pb_GetPhoneCalls"
	adoCmd.CommandType = adCmdStoredProc
	Set adoCmd.ActiveConnection = adoConn
  Set adoParam1 = adoCmd.CreateParameter("@P1_SSN", adInteger, adParamInput)
	adoCmd.Parameters.Append adoParam1
  Set adoParam2 = adoCmd.CreateParameter("@P2_CallBack", adChar, adParamInput, 1)
  adoCmd.Parameters.Append adoParam2
  Set adoParam3 = adoCmd.CreateParameter("@P3_LogonID", adVarChar, adParamInput, 30)
  adoCmd.Parameters.Append adoParam3
  Set adoParam4 = adoCmd.CreateParameter("@P4_Summary", adChar, adParamInput, 3)
  adoCmd.Parameters.Append adoParam4
  Set adoParam5 = adoCmd.CreateParameter("@P5_ACSUserID", adVarChar, adParamInput, 6)
  adoCmd.Parameters.Append adoParam5
  Set adoParam6 = adoCmd.CreateParameter("@P6_FromDate", adChar, adParamInput, 8)
  adoCmd.Parameters.Append adoParam6
  Set adoParam7 = adoCmd.CreateParameter("@P7_ThruDate", adChar, adParamInput, 8)
  adoCmd.Parameters.Append adoParam7
  Set adoParam8 = adoCmd.CreateParameter("@P8_FromTime", adVarChar, adParamInput, 20)
  adoCmd.Parameters.Append adoParam8
  Set adoParam9 = adoCmd.CreateParameter("@P9_ThruTime", adVarChar, adParamInput, 20)
  adoCmd.Parameters.Append adoParam9
  adoCmd("@P1_SSN") = m_iSSN
  adoCmd("@P2_CallBack") = m_sCallBack
  adoCmd("@P3_LogonID") = m_sLogonID
  adoCmd("@P4_Summary") = m_sSummCode
  adoCmd("@P5_ACSUserID") = m_sACSUserID
  adoCmd("@P6_FromDate") = m_sFromDate
  adoCmd("@P7_ThruDate") = m_sThruDate
  adoCmd("@P8_FromTime") = m_sFromTime
  adoCmd("@P9_ThruTime") = m_sThruTime
end if
%>
<html>
<head>
...
<%
      ...
      if bUseSQL and iRowCount > 1 then
        sTemp = "<table width='100%' border='0'><tr><td align='right'><font size=1 face='verdana, arial, helvetica'><b>This is phone record <font color='sienna'>" & iRowCount - iRemainder & "</font> out of a possible " & adoRS.RecordCount & ".</b></font></td></tr></table>"
			  'sTemp = "<p><font size=1 face='verdana, arial, helvetica'><b>This is phone record <font color='sienna'>" & iRowCount - iRemainder & "</font> out of a possible " & adoRS.RecordCount & ".</b></font><br>"
        'Response.Write sTemp
      end if
...
<%   
                      if LCASE(MID(adoRS("UserName"), INSTRREV(adoRS("UserName"), " ") +1)) = LCASE(MID(Session("User"), 2)) then 
%>
                        <input TYPE="BUTTON" ID="SaveCall" VALUE="Save Changes" onClick="UpdateCall <%= i%>,0,&quot;<%= adoRS("SSN")%>&quot;,&quot;<%= adoRS("TimeofCall")%>&quot;">

<%                    else 
%>
                        <%= Session("NWAName") %> is not authorized to make changes to this phone record.
<%                      
                      end if
%>
                    </td>
